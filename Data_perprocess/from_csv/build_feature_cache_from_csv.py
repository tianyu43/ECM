...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build year feature caches from sample-feature CSV files.")
...
def main() -> None:
    ...
    cfg = CacheBuildConfig(
        ...
        seed=args.seed,
        sample_mode=f"csv_{args.sensor.lower()}",
    )
    paths = project_paths()
    output_root = paths.processed_data_dir / "from_csv" / cfg.site
    output_root.mkdir(parents=True, exist_ok=True)
    root_logger = Logger(output_root / f"build_{cfg.site_token}_{args.sensor.lower()}_feature_cache_from_csv.log")
    root_logger.log(f"building csv feature cache config={vars(args)}")
    ...
    root_logger.log("csv feature cache build complete")
