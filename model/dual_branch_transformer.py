...
@dataclass
class DualBranchEncoderOutput:
    features: Tensor
    tokens: Tensor
    token_mask: Tensor


class CyclicDoyEncoding(nn.Module):
    ...
    def forward(self, doy: Tensor) -> Tensor:
        angle = 2.0 * math.pi * doy.float().clamp(1, 366) / 366.0
        features = torch.stack([angle.sin(), angle.cos()], dim=-1)
        return self.proj(features)


class LogDeltaEncoding(nn.Module):
    def __init__(self, d_model: int, hidden_dim: Optional[int] = None) -> None:
        super().__init__()
        hidden = hidden_dim or d_model
        self.proj = nn.Sequential(
    ...
    def forward(self, delta: Tensor) -> Tensor:
        features = torch.log1p(delta.float().clamp_min(0.0)).unsqueeze(-1)
        return self.proj(features)
...
class SensorBranch(nn.Module):
    ...
    def __init__(
        ...
        self.doy_encoder = CyclicDoyEncoding(d_model)
        self.delta_encoder = LogDeltaEncoding(d_model)
    ...
    def forward(self, x: Tensor, obs_mask: Tensor, doy: Tensor) -> Tensor:
        ...
        tokens = tokens + self.doy_encoder(doy) + self.delta_encoder(delta)
...
class SensorFusion(nn.Module):
    ...
    def forward(self, s1_tokens: Tensor, s2_tokens: Tensor, s1_token_mask: Tensor, s2_token_mask: Tensor) -> Tensor:
        ...
        token_mask = (s1_token_mask | s2_token_mask).unsqueeze(-1).float()
        return self.out(fused) * token_mask
...
class DualBranchS1S2Transformer(nn.Module):
    ...
    def forward(self, x: Tensor, obs_mask: Optional[Tensor] = None, doy: Optional[Tensor] = None) -> DualBranchEncoderOutput:
        if x.ndim != 3:
        ...
        return DualBranchEncoderOutput(features=pooled, tokens=fused_tokens, token_mask=token_mask)
